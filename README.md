...
3. After that, final.py allows user to run music from YouTube and Apple Music. Say "Jarvis", turn on Travis Scott as example, which will open youtube. Moreover, you can switch languages by saying key words, without having to restart the esp32 initialization with buttons all over again. There is additional command for macOS users, as I added a feature to open Apple Music. To see the full list of commands, see def parse_and_execute_command.
